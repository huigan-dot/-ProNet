...
class MatchingNetwork(nn.Module):
    def __init__(self, n: int, k: int, q: int, fce: bool, num_input_channels: int,
        ...
        # Arguments:
            n: Number of examples per class in the support set  n：支持集中每个类的示例数
            k: Number of classes in the few shot classification task  k：少镜头分类任务中的类数
            q: Number of examples per class in the query set  q：查询集中每个类的示例数
        ...
        #选择是否添加 BidrectionalLSTM和 AttentionLSTM层
        if self.fce:
            self.g = BidrectionalLSTM(lstm_input_size, lstm_layers).to(device, dtype=torch.double)#实现注意力
...
class BidrectionalLSTM(nn.Module):
    def __init__(self, size: int, layers: int):
        ...
        # 如可以解释成:layers层的LSTM,输入的每个词用100维向量表示,隐藏单元和记忆单元的尺寸是20
...
class AttentionLSTM(nn.Module):
    def __init__(self, size: int, unrolling_steps: int):
        ...
        # 该模块构建LSTM中的一个Cell，同一层会共享这一个Cell，但要手动处理每个时刻的迭代计算过程。
