...
def meta_gradient_step(model: Module,
    ...
    for meta_batch in x:
        ...
        # Create a fast model using the current meta model weights  保存打印每一次迭代元素的名字和param
        # python中的字典是无序的，因为它是按照hash来存储的 python中有个模块collections(英文，收集、集合)，里面自带了一个子类OrderedDict，实现了对字典对象中元素的排序
        fast_weights = OrderedDict(model.named_parameters())# 通过 named_parameters 能够获取到所有的参数保存为
        # print(fast_weights['logits.weight'].shape)# torch.Size([5, 448])
        # print(fast_weights)
        # print(fast_weights['logits.bias'].shape) # torch.Size([5])
        # Train the model for `inner_train_steps` iterations
        for inner_batch in range(inner_train_steps):
            # Perform update of model weights
            # 采用训练任务的的support集更新模型参数  模型的训练
        ...
        # query实现模型参数初始化参数的真正更新
        y = create_nshot_task_label(k_way, q_queries).to(device)
        logits = model.functional_forward(x_task_val, fast_weights)#前向传播
        loss = loss_fn(logits, y)
        ...
        # 计算准备率
        acc = np.asarray([torch.argmax(logits, -1).cpu().numpy() == y.cpu().numpy()]).mean()
        task_acc.append(acc)
    ...
    if order == 1:
        ...
        return torch.stack(task_losses).mean(), torch.cat(task_predictions),task_acc
        ...
        if train:
            ...
            optimiser.step()
        task_acc = np.mean(task_acc)
        return meta_batch_loss, torch.cat(task_predictions),task_acc
    else:
